...
const typeEnum = z.enum(VALID_CONTENT_TYPES);
// ─── Health ───
...
app.put("/api/v1/content/:id", userMiddleware, async (req, res) => {
    ...
    if (typeof parsed.data.collectionName === "string")
        update.collectionName = parsed.data.collectionName.trim();
...
// Bulk delete
app.post("/api/v1/content/bulk-delete", userMiddleware, async (req, res) => {
...
// ════════════════════════ SHARE ════════════════════════
app.get("/api/v1/share/status", userMiddleware, async (req, res) => {
...
app.get("/api/v1/brain/:shareLink", async (req, res) => {
    ...
    if (!user)
        return res.status(404).json({ message: "Owner no longer exists" });
    res.json({ username: user.username, content });
});
// ════════════════════════ AI ════════════════════════
app.post("/api/v1/ai/search", userMiddleware, async (req, res) => {
    try {
...
app.post("/api/v1/ai/search-public", async (req, res) => {
    try {
        ...
        if (!query)
            return res.status(400).json({ message: "Search query is required" });
        if (!hash)
            return res.status(400).json({ message: "Share hash is required" });
...
async function performAiSearch(query, contents) {
    ...
    try {
        ...
        return {
            matchedIds: Array.isArray(parsed.matchedIds)
                ? parsed.matchedIds.filter((id) => typeof id === "string" && validIds.has(id))
                : [],
...
async function performAiChat(message, history, contents) {
    ...
    try {
        ...
        return {
            ...
            sourceIds: Array.isArray(parsed.sourceIds)
                ? parsed.sourceIds.filter((id) => typeof id === "string" && validIds.has(id))
                : [],
        };
    }
    catch {
